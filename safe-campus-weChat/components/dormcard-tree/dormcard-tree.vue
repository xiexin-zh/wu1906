	...
	methods: {
		loadNode(node, resolve) {
			if (node.level === 0) {
				...
				$ajax
					.post({
						url: `${hostEnv.protal_api_11006}/dorm/controller/list`,
						params: {
							schoolCode: this.schoolInfo.schoolCode,
							deviceType: '3',
							buildingCode: node.data.code,
							page: 1,
							size: 999
						}
					})
		...
		confirm() {
			...
			console.log(this.selectedData.filter(item => item.type === '2'))
