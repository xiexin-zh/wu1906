...
<template>
	<view class="date">
		...
		<swiper :style="{height:(retract ? 2 * 58 : (week_list.length + 1) * 58 ) + 'rpx'}" :current="current" circular @change="change_date">
			<swiper-item>
				...
				<view class="date_dl" v-show="!retract || index == to_prev_week_index" v-for="(item,index) in   week_list_prev_co" :key="index">
					<view class="dd" @click="item_click(vo,index,key)" v-for="(vo,key) in item" :key="key">
						<view class="num" :class="[vo.today ? 'today' : '',vo.type == 'month' ? 'month' : (retract ? '' : 'disabled')]">{{vo.day > 9 ? vo.day : `0${vo.day}`}}</view>
...
.head{
	display: flex; align-items: center; height: 70rpx; justify-content: center;
	color:#fff;
	.title{
...
.retract{
	display: flex; justify-content: center; align-items: center; height: 30rpx;;
	.iconfont{
...
.date_dl{
	...
	.dd{
		...
		position: relative;
		.num{
			width: 50rpx; height: 50rpx; border-radius: 50%; line-height: 50rpx; 
			&.disabled{
