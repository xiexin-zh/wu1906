<template>
  <view class="container">
    ...
    <view class="set_box" v-if="detail.repairCopyList && detail.repairCopyList.length !== 0">
      <view class="set_time">
        <view></view>
      </view>
      <view class="set-1">
    ...
    <view class="set_box" v-for="(item, index) in detail.dataList" :key="index">
      <view class="set_time">
        ...
        <view class="set-2">
          <view class="right u-fx-ver">
            <text
              ...
              <view v-if="item.repairState === '2'"
                >退回理由：{{ item.remark }}</view
              >
            </text>
            <text class="detail" v-if="item.type === '2' || item.type === '3'"
              >处理人： {{ item.previousUserName }}
              <text class="set-state"> 已指定 </text>
            </text>
          </view>
          <view class="detail" v-if="item.type === '2' || item.type === '3'">
            指定人：<text class="pa-4"
              >{{ item.approvalUserName
              }}<text class="set-state"> 代审批 </text></text
            ></view
          >
        </view>
      </view>
