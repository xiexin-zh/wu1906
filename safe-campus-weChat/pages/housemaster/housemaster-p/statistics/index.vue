	...
	<view class="qui-page">
		...
		<view class="head">
			<button type="default" @click="onShowDatePicker('rangetime')">
				<text
					v-if="this.rangetime.length !== 0"
				>{{ rangetime[0] | gmtToDate('date') }}~{{ rangetime[1] | gmtToDate('date') }}</text>
		...
		<view class="record-box">
			<view class="attandence-box">
				<view class="attandence-info qui-fx-ac-jc" v-for="item in attandenceInfo" :key="item.id">
		...
		<view class="tab_">
			<tab-list tabList="1234"></tab-list>
		</view>
		<view class="list">
			...
			<no-data msg="暂无查寝记录~" v-if="recordList.length === 0"></no-data>
