<template>
	<view class="face-student u-bg-fff">
		...
		<cp-select-item v-if="isGX" @confirm="confirmSubject" v-model="subjectTag" :items="subjectList"></cp-select-item>
		...
		<view class="tab-select u-bd-b u-fx-ac">
			<view class="u-fx-f1 u-fx-ac u-fx-jc u-bd-r" @click="gradeTag = true">
				{{ gradeInfo.label }}
				<view class="rit-icon"></view>
			</view>
			<view v-if="isGX" class="u-fx-f1 u-fx-ac u-fx-jc u-bd-r" @click="subjectTag = true">
				{{ subjectInfo.label }}
				<view class="rit-icon"></view>
			</view>
			<view class="u-fx-f1 u-fx-ac u-fx-jc u-bd-r" @click="classTag = true">
				{{ classInfo.label }}
				<view class="rit-icon"></view>
			</view>
		</view>
		...
		<scroll-view scroll-y="true" class="scroll-h u-bg-fff">
			...
			<view @click="showPhoto(list)" class="student-list u-bd-r u-fx-ac-jc" v-for="list in studentList" :key="list.userCode">
				...
				<view class="name u-font-01">{{ list.userName }}</view>
			</view>
		</scroll-view>
	</view>
</template>
...
export default {
	...
	async created () {
		if(this.userInfo.schoolType === '8' || this.userInfo.schoolType === '9'){
			this.isGX = true
			const res = await actions.highSchoolGradeList({
	...
	methods: {
		...
		async confirmGrade (item) {
			...
			if(this.isGX){
				this.subjectInfo = {
					...
					const res = await actions.highSchoolSubjectList({
						schoolCode: this.userInfo.schoolCode,
		...
		async confirmSubject (item) {
			if (!item) return
			this.subjectInfo = item
			this.classInfo = {
				label: '请选择班级'
			}
			const res = await actions.highSchoolClassList({
				schoolCode: this.userInfo.schoolCode,
				gradeName: this.gradeInfo.label,
				subjectCode: item.key
			})
