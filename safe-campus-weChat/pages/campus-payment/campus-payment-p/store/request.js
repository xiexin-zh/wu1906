...
uniRequest.defaults.headers.Authorization = window.localStorage.getItem('Authorization')
// 请求拦截
...
uniRequest.interceptors.request.use(
  function (config) {
    return new Promise((resolve) => {
      if (config.url.indexOf('billInfo/info') !== -1) {
        let id = config.url.split('/')[config.url.split('/').length - 1]
        if (isRefreshing) {
          refreshTokenRequst(id)
        }
        isRefreshing = false
        resolve(config)
      } else {
...
function refreshTokenRequst(id) {
  uni.request({
    url: `/protal_api_10097/billInfo/info/${id}`,
    method: 'get',
    success: (response) => {
      const Authorization = 'bearer' + ' ' + response.header.token
      const callbackUrl = response.header.callbackurl
