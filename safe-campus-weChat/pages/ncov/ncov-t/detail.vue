<template>
	<view class="set u-page u-fx-ver u-fff">
		<view class="u-fx-ac u-fx-jsb set-info div-shadow">
			<view class="set-img"><image :src="detailInfo.photoUrl || '/mobile-img/person-auto.png'" alt="" /></view>
			<view class="u-fx-ver">
		...
		<view class="set-info div-shadow">
			<u-cell-group :border="false">
				<u-cell-item :arrow="false" title="体温" :value="`${detailInfo.temperature}℃`" />
				<u-cell-item :arrow="false" title="发热状态" :value="detailInfo.enableFever ? '发热' : '正常'" :value-style="{ color: detailInfo.enableFever ? '#fa3534' : '#19be6b' }" />
...
export default {
	...
	async mounted() {
		...
		let res = null
		if (this.id) {
			res = await actions.getReportDetail(this.id);
		}
		if (this.userCode) {
		  res = await actions.getReportDetailByUserCode({
