<template>
	<view class="bind-child">
		<u-popup v-model="isSchool" height="650" mode="bottom">
			<view @tap="chooseSchool(school)" class="u-padd-30 u-bd-b" v-for="(school, index) in schoolList" :key="index">
				{{ school.schoolName }}
			</view>
		</u-popup>
		...
		<view v-if="isShowParent">
			<view class="form-item u-fx-ac">
				<view>手机号</view>
				<view class="u-fx-f1">
					<u-input input-align="right" v-model="formData.mobile" placeholder="请输入手机号" trim type="number" ></u-input>
				</view>
				<phone-code :mobile="formData.mobile"></phone-code>
			</view>
			<view class="form-item u-fx-ac">
				<view>验证码</view>
				<view class="u-fx-f1">
					<u-input input-align="right" v-model="formData.captchaCode" placeholder="请输入验证码" trim type="number" ></u-input>
				</view>
			</view>
			<view class="u-fx-ac u-padd-t u-padd-b u-bd-b">
				<view>上传头像</view>
				<view class="u-fx-f1 u-mar-l40">
					<upload-photo v-model = "fileList"></upload-photo>
				</view>
			</view>
		</view>
...
<script>
	...
	import UploadPhoto from '../component/UploadPhoto.vue'
		...
		methods: {
			...
			formYz (callback) {
				if (this.formData.schoolName === '请选择学校') {
					...
					return
				} else if (this.formData.relationShip === '请选择家属关系') {
