<template>
	<view class="one-card u-page u-fx-f1">
		<u-modal v-model="isShowTag" title="消费规则" show-cancel-button @confirm="editRule">
			<view class="slot-content form u-padd-t20">
				<view class="u-fx-ac u-bd-b item-list">
					<view class="u-font-01">每日消费限额：</view>
					<view class="u-fx-f1 u-mar-r10">
						<input
							type="digit"
							@input="everydayConsumeInput"
							class="item-input"
							v-model="accountInfo.everydayConsume"
				...
				<view class="u-fx-ac item-list">
					<view class="u-font-01">单次消费限额：</view>
					<view class="u-fx-f1 u-mar-r10">
						<input type="digit" @input="singleConsumeInput" class="item-input" v-model="accountInfo.singleConsume" style="text-align: right;" placeholder="请输入单次消费限额" />
					</view>
				</view>
			</view>
		</u-modal>
		...
		<view v-else>
			<view class="user-info u-fx-ac u-mar-b10 u-padd-40 u-bd-b u-content-color u-bg-fff">
				...
				<view class="u-fx-f1 u-line3 u-mar-l20">
					...
					<view class="u-font-01 u-type-white">ID：{{ accountInfo.accountNo }}</view>
				</view>
				<view class="u-line3">
			...
			<view class="content">
				<view class="title u-fx-jsb u-padd-20 u-padd-b10">
					...
					<view class="u-type-primary u-mar-r10" @click="isShowTag = true">消费规则 ></view>
				</view>
...
export default {
	...
	methods: {
		...
		async showList(tag = false) {
			...
			this.morePage = res.total > this.dataList.length;
		},
		// 下拉刷新
		loadMore() {
			if (!this.morePage) {
		...
		singleConsumeInput(event) {
			this.accountInfo.singleConsume = Math.floor(event.target.value * 100) / 100
		},
		everydayConsumeInput(event) {
			this.accountInfo.everydayConsume = Math.floor(event.target.value * 100) / 100
		},
		async editRule() {
			await actions.updateConsumeRule({
				everydayConsume: this.accountInfo.everydayConsume,
				singleConsume: this.accountInfo.singleConsume,
				userCode: this.userCode
			});
