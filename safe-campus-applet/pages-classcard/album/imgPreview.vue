		...
		<view class="mask">
			<swiper @change="changeSwiper" class="my_swiper" :current="current" :circular="circular" :indicator-dots="indicatorDots" :autoplay="autoplay" :duration="duration">
				<swiper-item v-for="(item, i) in picList" :key="i">
						<view v-if="item.fileType !== 'video'" class="bg_img" :style="{ backgroundImage: 'url('+item.photoSrc+')'}"></view>
						<view class="video u-fx-ac" v-else>
							<video class="file-video" :src="item.photoSrc" controls enable-play-gesture></video>
						</view>
