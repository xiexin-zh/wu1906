	...
	<view class="content-circle u-padd-20 u-border-radius u-auto">
		<view v-if="parseInt(unReadCommentInfo.unReadCommentCount) > 0" class="u-type-white-bg u-padd-t20 u-fx-jc u-border-radius">
			<view class="new-tips u-fx-ac u-type-white u-bg-000 u-padd-10 u-border-radius" @tap="goNewList">
				...
				<text class="u-font-01">{{ unReadCommentInfo.unReadCommentCount }}条新消息</text>
			</view>
		</view>
		<view class="content-circle-box u-bd-b u-fx u-padd-20 u-bg-fff u-border-radius" v-for="(item, index) in newsList" :key="index">
			<view class="headimg"><image class="img u-border-radius" :src="item.photo_url || 'http://canpointtest.com/mini-img/auto-user-icon.png'"></image></view>
			<view class="content u-fx-f1 u-padd-l20 u-padd-r20">
				<view class="u-type-primary-dark u-font-1">
				...
				<view class="content-img u-mar-t10" v-if="item.fileList.length > 0">
					<!-- //只有一张图时候 -->
					<view v-if="item.fileList.length === 1"><image mode="widthFix" class="img-1" :src="item.fileList[0].url" @tap="previewImg(0, item.fileList)"></image></view>
					<!-- 有多张图的时候 -->
					<view v-else>
						<view class="content-img-more u-fx-wp">
							<image
								...
								:src="file.url"
								@tap="previewImg(index, item.fileList)"
							></image>
						</view>
					</view>
				</view>
				...
				<view class="common u-fx-jsb u-fx-ac u-font-01 u-mar-t10">
					<view class="u-tips-color">
						...
						<text v-if="userType === 1 || (userType === 2 && item.create_userCode === userCode ) || (userType === 3 && item.children_code === userCode )" class="u-type-primary-dark u-mar-l10 u-font-01" @tap="delMoment(item.id)">删除</text>
					</view>
					<transition name="fade">
						<view v-show="item.showAction" class="show-action u-fx-ac u-bg-color u-padd-10 u-border-radius">
							<view class="u-fx u-font-01" @tap="clickThumb(item, index)">
							...
							<view class="u-fx u-font-01" @tap="handleComment(item, null)">
				...
				<!-- 点赞人 评论 -->
				<view class="msg-box u-bg-color u-mar-t10 u-type-primary-dark u-font-01">
					...
					<view class="u-padd-20 thumbinfo " v-if="item.commentList.length > 0">
						<view class="u-padd-b10" v-for="(comment, index) in item.commentList" :key="index">
							...
							<text class="u-content-color" @tap="handleComment(item, comment)">{{ comment.msg }}</text>
							<text v-if="userType === 1 || (comment.childrenCode ? comment.childrenCode === userCode : comment.createUsercode === userCode )" class="u-type-primary-dark u-mar-l10 u-font-01" @tap="delComment(comment.id)">删除</text>
						</view>
					</view>
				</view>
			</view>
...
created() {
		...
		this.userCode = this.userType === 3 ? this.currentBindStudent.userCode : this.userInfo.userCode;
	},
	methods: {
		//查看大图
		...
		clickThumb(item, index) {
			if(item.isLike){
				...
				const req = {
					...
					createUsername: this.userType === 3 ? this.currentBindStudent.userName : this.userInfo.userName,
					momentsId: item.id,
