        ...
        <div class="resultSearch">
          <div class="search">
            <el-input
              ...
              @clear="getTestResult"
            ></el-input>
            <span class="searchBtn" @click="getTestResult">搜索</span>
          </div>
        ...
        <div class="resultListBox">
          <div class="resultList">
            <div class="resultDetail">
              ...
              <span class="detailTitle" @click="showList">結果詳情</span>
            </div>
            <ul v-show="detailShow" class="listBox">
              <li v-for="(item, index) in resultList" :key="index">
                <p class="itemTitle">
                  ...
                  ></span>
                  <span @click="showListItem(item.module)">
                    {{
                    item.module
                    }}
                  </span>
                  <span @click="showListItem(item.module)">
                    {{
                    formatPoint(item.passed,item.coverage)
                    }}
                  </span>
                </p>
                <div v-show="item.show">
                  <div class="ListItem" v-for="(li, i) in item.data_list" :key="i">
                    <span v-if="li.result == 1" class="iconfont icon-gou1 passed"></span>
                    <span v-if="li.result == 0" class="iconfont icon-chacha failed"></span>
                    <span v-if="li.result == 2" class="iconfont icon-tanhao error"></span>
                    <p>
          ...
          <div class="resultEchat">
            <div id="resultChart" @click="toBugList"></div>
...
export default {
  data() {
    return {
      ...
      static_data: [],
      chartList: [
        { title: "腳本開發中", color: "#949494" },
        { title: "包含嚴重Bug", color: "#FC4E4E" },
        { title: "包含中度Bug", color: "#F7A56A" },
        { title: "包含輕度Bug", color: "#FFD63B" },
        { title: "無Bug", color: "#7ED321" }
      ]
    };
  },
  computed: {
    color() {
      function getColor(coverage, severity) {
        if (coverage == 0) {
          return "#949494";
        }
        if (severity == 1) {
      ...
      let colorArr = this.static_data.map(item => {
        return getColor(item.coverage, item.severity);
      });
      return colorArr;
    },
    echartData() {
  ...
  methods: {
    ...
    // 獲取測試結果數據
    async getTestResult() {
      const res = await DataAction.getResultData({
      ...
      if (res.code == 200) {
        const { module_data, static_data } = res.data;
