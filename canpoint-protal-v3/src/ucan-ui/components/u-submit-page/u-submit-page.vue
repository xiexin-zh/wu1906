<template>
  <div>
    ...
    <a-form :form="form">
      <div v-for="(item, index) in formData" :key="index">
        ...
        <a-form-item
          ...
          v-if="item.type === 'area'"
        >
          <u-choose-area v-model="areaList" :placeholder="item.initValue"></u-choose-area>
        </a-form-item>
        <!--地区选择-->
        <!--组织机构-->
        <a-form-item
          ...
          v-if="item.type === 'org'"
        >
          <u-choose-org v-model="orgList" :default-value="item.initValue" :school="schoolCode"></u-choose-org>
        </a-form-item>
        <!--组织机构-->
        <!--单个日期-->
    ...
    <a-row class="u-mar-t" v-if="isButton">
      <a-col :span="labelCol"></a-col>
      <a-col :span="wrapperCol">
        <u-space>
          <u-button @click="cancel" type="default">取消</u-button>
          <u-button @click="submitOk" :loading="confirmLoading" type="primary">确定</u-button>
        </u-space>
      </a-col>
    </a-row>
  </div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      imgList: (this.formData.filter(item => item.type === 'uploadImg')[0] || {}).initValue,
