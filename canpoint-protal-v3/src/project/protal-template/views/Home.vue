<template>
  <div class="home page-layout u-fx-ver u-type-white-bg">
    <u-space direction="vertical" class="layout-padd">
      <u-alert closable type="error" message="温馨提示" description="在此界面可以新增和删除学生"></u-alert>
      <u-search-form :search-label="searchLabel" @search="search">
        <div slot="left">
          <u-space>
            <u-button icon="plus" @click="change" type="primary">新增设备</u-button>
      ...
      <u-space class="u-fx-ac">
        ...
        <u-button type="danger" :disabled="chooseList.length === 0">批量解绑班主任</u-button>
        <u-button type="danger" :disabled="chooseList.length === 0">批量解绑教师</u-button>
      </u-space>
    </u-space>
    <div class="u-fx u-fx-f1">
      ...
      >
        <template v-slot:slotOne="slotOne">
          <u-tag type="primary">{{ slotOne.record.ycq }}</u-tag>
        </template>
        <template v-slot:slotTwo="slotTwo">
          <u-tag pointer type="warning-bd">{{ slotTwo.record.cq }}</u-tag>
        </template>
        <template v-slot:slotThree="slotThree">
          <u-tag pointer type="success-bd">{{ slotThree.record.gq }}</u-tag>
        </template>
        <template v-slot:slotFour="slotFour">
          <u-type>{{ slotFour.record.zplx }}</u-type>
        </template>
        <template v-slot:slotFive="slotFive">
          <u-type>{{ slotFive.record.jclx }}</u-type>
        </template>
