<template>
  <div class="template page-layout u-fx-ver">
    <u-space direction="vertical">
      ...
      <div class="u-type-white-bg layout-padd">
        <u-space class="u-fx-ac">
          已选择 {{ chooseList.length }} 项数据
          <u-button type="light">排序</u-button>
          <u-button :disabled="chooseList.length === 0" type="danger">批量解绑班主任</u-button>
          <u-button :disabled="chooseList.length === 0" type="danger">批量解绑教师</u-button>
...
import TemplateH from './template-com/TemplateH'
import TemplateS from './template-com/TemplateS'
const searchLabel = [
  {
    value: 'name', // 表单属性
    type: 'input', // 表单类型
...
export default {
  name: 'Home',
  components: { TemplateH, TemplateS },
  data() {
    return {
      searchLabel,
      userList: [],
