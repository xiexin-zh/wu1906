...
export default {
  ...
  computed: {
    ...
    fieldList() {
      if (this.type == 'customer') {
        return [
          { name: '戶口名稱', field: 'name' },
          { name: '跟進人', field: 'owner_user_id' }
        ]
      } else if (this.type == 'contacts') {
        return [
          { name: '姓名', field: 'name' },
          { name: '戶口名稱', field: 'customer_name' },
          { name: '跟進人', field: 'owner_user_id' }
        ]
      } else if (this.type == 'leads') {
        return [
          { name: '線索名稱', field: 'name' },
          { name: '跟進人', field: 'owner_user_id' }
        ]
      }
      return []
  ...
  methods: {
     ...
     */
    getConditionList() {
      let customerTips = `為確保戶口信息安全性，戶口名稱字段增加了防作弊機制，以<br />
                下關鍵詞為禁搜關鍵詞：<br />
                1.省、市名；<br />
                2.中國、公司、有限公司、有限責任公司、股份有限公司；`
      let leadsTips = `為確保戶口信息安全性，戶口名稱字段增加了防作弊機制，以<br />
                下關鍵詞為禁搜關鍵詞：<br />
                1.省、市名；<br />
                2.中國、公司、有限公司、有限責任公司、股份有限公司；`
      if (this.type == 'customer') {
        return [
          ...
          { name: '手機', field: 'mobile', value: '' },
          { name: '電話', field: 'telephone', value: '' }
        ]
      } else if (this.type == 'contacts') {
        return [
          { name: '自然人', field: 'name', value: '' },
          {
            name: '戶口名稱',
            field: 'customer_name',
            value: '',
