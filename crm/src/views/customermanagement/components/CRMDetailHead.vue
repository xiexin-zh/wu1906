<template>
  <div style="position: relative;">
    <flexbox class="t-section">
      <img class="t-img" :src="avatarIcon" />
    ...
    <flexbox class="h-section" align="stretch" v-show="flag">
      <flexbox-item
        class="h-item"
        span="240"
...
export default {
  ...
  computed: {
    ...
    avatarIcon() {
      const { avatar } = this.detail;
      return avatar ? avatar : require("@/assets/no-head-img.svg");
    },
    // 展示转移
    showTransfer() {
      if (
  ...
  mounted() {
    ...
    if (flagItem.includes(this.currentHash)) {
      this.flag = false;
    }
  },
  methods: {
