<template>
  <el-dialog
    ...
    width="800px"
  >
    <div style="margin-bottom: 10px;">篩選條件</div>
      ...
      <el-form-item>
        <template v-for="(formItem, index) in form">
          <el-row :key="index">
            ...
            <el-col
              ...
              v-if="
                formItem.form_type !== 'date' &&
                  ...
                  formItem.form_type !== 'business_type'
              "
              >&nbsp;</el-col
            >
            <el-col
              ...
              "
            >
              <el-select v-model="formItem.condition" placeholder="請選擇範圍">
                <el-option
            ...
            <!-- 商机组 -->
            <el-col :span="1" v-if="formItem.form_type == 'business_type'"
              >&nbsp;</el-col
            >
            <el-col :span="4" v-if="formItem.form_type == 'business_type'">
              <el-select
            ...
            >
              ...
              <el-date-picker
                ...
                "
                :type="
                  formItem.form_type === 'date' ? 'daterange' : 'datetimerange'
                "
    ...
    <p class="el-icon-warning warning-info" v-show="showErrors">
      <span class="desc">篩選條件中有重複項！</span>
    </p>
    <el-button type="text" @click="handleAdd">+ 添加篩選條件</el-button>
    <div class="save" v-if="!isSeas" v-show="!Object.is(crmType, 'customer')">
      <el-checkbox v-model="saveChecked">保存為場景</el-checkbox>
      <el-input
        class="name"
        v-show="saveChecked"
        v-model.trim="saveName"
        :maxlength="10"
        placeholder="請輸入場景名稱，最多10個字符"
      >
      </el-input>
      <div class="save-setting" v-show="saveChecked">
        <el-checkbox v-model="saveDefault">設置為默認</el-checkbox>
      </div>
    </div>
  ...
  methods: {
    ...
    calConditionOptions(form_type, item) {
      if (
        ...
        return [
          { value: "is", label: "等於", disabled: false },
          { value: "isnot", label: "不等於", disabled: false }
        ];
      } else if (
        form_type == "module" ||
        form_type == "text" ||
        ...
        return [
          ...
          { value: "contains", label: "包含", disabled: false },
          { value: "not_contain", label: "不包含", disabled: false },
          { value: "is_empty", label: "為空", disabled: false },
          { value: "is_not_empty", label: "不為空", disabled: false },
          { value: "gt", label: "大於", disabled: false },
          { value: "egt", label: "大於等於", disabled: false },
          { value: "lt", label: "小於", disabled: false },
          { value: "elt", label: "小於等於", disabled: false }
        ];
        ...
        return [
          ...
          { value: "contains", label: "包含", disabled: false },
          { value: "not_contain", label: "不包含", disabled: false },
          { value: "start_with", label: "開始於", disabled: false },
          { value: "end_with", label: "結束於", disabled: false },
          { value: "is_empty", label: "為空", disabled: false },
          { value: "is_not_empty", label: "不為空", disabled: false },
          { value: "gt", label: "大於", disabled: false },
          { value: "egt", label: "大於等於", disabled: false },
    ...
    handleConfirm() {
      if (this.showErrors) {
        this.$message.error("篩選條件中有重複項！");
        return;
      }
      if (this.saveChecked) {
        if (!this.saveName || this.saveName === "") {
          this.$message.error("場景名稱不能為空！");
          return;
        }
      }
      for (let i = 0; i < this.form.length; i++) {
        let o = this.form[i];
        if (!o.field || o.field === "") {
          this.$message.error("要篩選到字段名稱不能為空！");
          return;
        }
        if (o.form_type == "business_type") {
          if (!o.type_id && !o.status_id) {
            this.$message.error("請輸入篩選條件的值！");
            return;
          }
        } else if (
          o.form_type == "date" ||
