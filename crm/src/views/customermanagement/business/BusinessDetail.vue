  ...
  computed: {
    ...
    tabnames() {
      ...
      if (this.crm.product && this.crm.product.index) {
        tempsTabs.push({ label: '產品', name: 'product' })
      }
  ...
  methods: {
    ...
    handleBusinessStatus(data) {
      ...
      if (statusList) {
        ...
        if (data.is_end == 0) {
          ...
          overItem.name = '贏單'
          overItem.title = '贏單' // 详情标题 和 内容
          overItem.detail = '贏單率100%'
    ...
    handleStatuResult(item, index) {
      ...
      if (item.type == 2 || item.type == 3) {
        ...
        var message = '確定將當前商機設為' + item.name + '嗎?'
        this.$confirm(message, '提示', {
          confirmButtonText: '確定',
          cancelButtonText: '取消',
          type: 'warning'
