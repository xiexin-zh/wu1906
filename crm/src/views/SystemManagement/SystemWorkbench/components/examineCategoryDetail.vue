    ...
    <flexbox orient="vertical"
      ...
      <div class="header">
        ...
        <div class="detail-header-des">
          <div class="status">狀態：{{showData.status == 0 ? '停用' : '啟用'}}</div>
      ...
      <div class="detail-body">
          ...
          <div class="create-sections-content">
            <div class="dialog-content">
              <flexbox class="content-items">
                <div class="content-items-name">審批名稱</div>
                <div class="content-items-value">{{showData.title}}</div>
              </flexbox>
              <flexbox class="content-items">
                <div class="content-items-name">適用範圍</div>
                <div class="content-items-value">{{showData|formatedScopeInfo}}</div>
              </flexbox>
              <flexbox class="content-items">
                <div class="content-items-name">審批說明</div>
                <div class="content-items-value">{{showData.remark}}</div>
              </flexbox>
            </div>
          </div>
        </create-sections>
        <create-sections title="表單"
                         class="create-sections">
          <div slot="header"
               class="preview-section">
            <el-button type="text"
                       @click="handlePreview">預覽</el-button>
          </div>
        </create-sections>
        <create-sections title="流程"
          ...
          <div v-else
               class="create-sections-content">授權審批人</div>
        </create-sections>
      </div>
    </flexbox>
    <!-- 表单预览 -->
...
export default {
  ...
  filters: {
    ...
    toRowName: function(value) {
      return '第' + Nzhcn.encodeS(value) + '級'
    },
    // 标题描述
    toRowNameDes: function(data) {
      if (data.status == 1) {
        ...
        return data.user_id_info.length + '人或簽'
      } else if (data.status == 3) {
        return data.user_id_info.length + '人會簽'
  ...
  methods: {
    ...
    examineStatusChange() {
      ...
      // 启用停用
      this.$confirm(
        '您確定要' + (this.examineStatus === 0 ? '啟用' : '停用') + '該審批流?',
        '提示',
        {
          confirmButtonText: '確定',
          cancelButtonText: '取消',
