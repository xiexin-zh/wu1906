<template>
  <el-dialog :title="businessTitle"
    ...
    <div class="business-list">
      <div class="business-label">商機組名稱</div>
      <el-input v-model="name"
                style="width: 60%;"></el-input>
    </div>
    <div class="business-list">
      <div class="business-label">應用部門</div>
      ...
      <div class="business-des">默認全公司</div>
    </div>
    ...
    <span slot="footer"
      ...
      <el-button type="primary"
                 @click="businessSubmit">確 定</el-button>
...
export default {
  ...
  methods: {
    ...
    businessSubmit() {
      if (!this.name) {
        this.$message.error('請填寫商機組名稱')
      } else if (!this.businessDep) {
        this.$message.error('請關聯應用部門')
      } else {
        var pass = true
