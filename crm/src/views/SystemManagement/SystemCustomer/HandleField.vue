<template>
  <flexbox orient="vertical"
    ...
    <div class="title">{{'編輯'+getTitle()+'字段'}}</div>
    <el-container class="wrapper">
      <el-aside class="left">
        <div class="mini-title">字段庫</div>
        <ul>
      ...
      <el-container class="content"
        ...
        <el-header>
          ...
          <el-button type="primary"
                     style="padding: 8px 22px;border-radius:2px;background: #005DF7"
        ...
        <el-main>
          ...
          <p class="no-list"
             v-if="fieldArr.length == 0">從左側點擊或拖拽來添加字段</p>
        </el-main>
      </el-container>
      <el-aside class="right"
                width="310px">
        <div class="mini-title">字段屬性</div>
    ...
    handleSave() {
      ...
      for (let index = 0; index < tempFieldArr.length; index++) {
        ...
        if (!item.name) {
          ...
          this.$message({
            type: 'error',
            message: '第' + (index + 1) + '行的自定義字段，標識名不能為空'
          })
          break
