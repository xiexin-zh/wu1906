<template>
  <div class="field-info-section">
    <div class="item-section">
      ...
      <div class="input-tips"><span>*</span>標識名不能為空</div>
    </div>
    <div class="item-section">
      <div class="name">
        提示文字
      ...
      <el-input v-model="field.input_tips"
                ...
                :rows="3"
                :disabled="disabled"></el-input>
      <div class="input-tips"><span>*</span>顯示在標識名右側的說明文字</div>
    </div>
    ...
    <div class="item-check-section">
      <el-checkbox v-model="field.is_null"
                   :disabled="disabled">設為必填</el-checkbox>
    </div>
    <div class="item-check-section">
      <el-checkbox v-model="field.is_unique"
                   :disabled="disabled">設為唯一</el-checkbox>
    </div>
  </div>
</template>
  ...
  computed: {
    default_tips() {
      if (this.field.form_type == 'floatnumber') {
        return '貨幣的整數部分須少於10位，小數部分須少於2位'
      } else if (this.field.form_type == 'number') {
        return '數字的整數部分須少於12位，小數部分須少於4位'
      }
      return ''
    },
    /** 展示最大输入 */
    ...
    handleRadio(type, item, index) {
      ...
      if (type == 'add') {
        this.field.showSetting.push({
          value: '選' + (this.field.showSetting.length + 1)
        })
      } else if (type == 'remove') {
