  ...
  <div v-loading="loading"
    ...
    <div class="tabs-bar">
      <el-tabs v-model="tabType"
        ...
        <el-tab-pane label="部門目標設置"
                     name="department"></el-tab-pane>
        <el-tab-pane label="員工目標設置"
    ...
    <div class="handle-bar">
      ...
      <el-select v-model="typeSelect"
                 placeholder="請選擇">
        <el-option v-for="item in [{label:'合同金額', value:1},{label:'回款金額', value:2}]"
                   :key="item.value"
                   :label="item.label"
                   :value="item.value">
        </el-option>
      </el-select>
      <el-cascader placeholder="選擇部門"
                   change-on-select
                   :options="deptList"
    ...
    handleClick(type) {
      if (type == 'search') {
        ...
        let name = `${this.dateSelect} 年${
          { department: '部門目標', user: '員工目標' }[this.tabType]
        }.xlsx`
        FileSaver.saveAs(
