  ...
  <create-view :loading="loading"
    ...
    <flexbox direction="column"
      ...
      <flexbox class="crm-create-flex"
        ...
        <div v-show="currentPage == 2"
          ...
          <div style="padding: 0 20px; font-size: 12px;">
            ...
            <div class="examine-items-add"><span @click="examineItemsAdd">+ 添加審批層級</span></div>
            <div class="examine-add-des">
              <p><span class="examine-add-required">*</span>當選擇“跟進人主管”審批時。系統僅會通知跟進人主管，但跟進人所有上級（主管、主管的主管）均可審批。</p>
              <p><span class="examine-add-required">*</span>當選擇多個“指定用戶”審批時。如果指定用戶沒有權限查看對應的合同，系統不會通知其審批。 </p>
              <p><span class="examine-add-required">*</span>當選擇“指定用戶（任意一人）”表示指定用戶中任意一人審批即可。當選擇“指定用戶（多人會簽）”表示 指定用戶中所有人都要審批。</p>
            </div>
            <el-radio v-model="examineType"
                      :label="0">授權審批人</el-radio>
          </div>
        </div>
      </flexbox>
      <div v-if="currentPage == 1"
        ...
        <el-button class="handle-button"
                   type="primary"
                   @click.native="nextPage">下一頁</el-button>
      </div>
      <div v-if="currentPage == 2"
        ...
        <el-button class="handle-button"
                   type="primary"
                   @click.native="currentPage = 1">上一頁</el-button>
      </div>
    </flexbox>
  </create-view>
</template>
      ...
      examineList: [
        {
          ...
          options: [
            { name: '跟進人主管', value: 1 },
            { name: '指定用戶（任意一人）', value: 2 },
            { name: '指定用戶（多人會簽）', value: 3 }
          ]
        }
      ]
    }
  ...
  methods: {
    ...
    getField() {
      ...
      field.push({
        ...
        name: '流程說明',
        setting: [],
        input_tips: '請填寫相關注意事項，方便員工在申請時查閱，限制輸入2000字',
    ...
    selectOptionsFocus(item, index) {
      if (this.examineList.length > 1) {
        ...
        var options = [{ name: '跟進人主管', value: 1 }]

        if (!removeTwo) {
          options.push({ name: '指定用戶（任意一人）', value: 2 })
        }
        if (!removeThree) {
          options.push({ name: '指定用戶（多人會簽）', value: 3 })
        }
        if (!removeFour) {
          options.push({ name: '上一級審批人主管', value: 4 })
        }
