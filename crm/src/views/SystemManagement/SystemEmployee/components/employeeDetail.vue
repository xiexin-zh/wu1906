          ...
          <div class="dialog-btn-group">
            ...
            <el-dropdown trigger="click"
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="reset">重置密碼</el-dropdown-item>
                <el-dropdown-item command="status">{{data.status === 0 ? '激 活' : '禁 用'}}</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </div>
          <div class="dialog-remark">
            <p>賬號狀態：{{{'0':'禁用','1':'激活','2':'未激活' }[data.status]}}</p>
            <p>創建時間：{{data.create_time | filterTimestampToFormatTime('YYYY-MM-DD')}}</p>
          </div>
        </div>
  ...
  data() {
    return {
      detailList: [
        ...
        { field: 's_name', value: '部門', type: 'select' },
        { field: 'post', value: '崗位' },
        { field: 'parent_name', value: '直屬上級', type: 'select' },
