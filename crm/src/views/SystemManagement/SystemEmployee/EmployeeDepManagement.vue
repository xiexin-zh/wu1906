  ...
  <div class="employee-dep-management">
      ...
      <div class="system-view-table flex-index">
        <div class="table-top"
          ...
          <el-button type="primary"
                     class="rt"
                     @click="newBtn">新建員工</el-button>
        </div>
        <flexbox v-if="selectionList.length > 0"
                 class="selection-bar">
          <div class="selected—title">已選中<span class="selected—count">{{selectionList.length}}</span>項</div>
    ...
    <el-dialog :visible.sync="depCreateDialog"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="submitDialog">確 定</el-button>
      </span>
    </el-dialog>
    <!-- 详情 -->
    ...
    <el-dialog title="重置密碼"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="passSubmit(passForm)">確 定</el-button>
      </span>
    </el-dialog>

    <!-- 重置登录账号 -->
    <el-dialog title="重置登錄賬號"
      ...
      <div class="el-password">
        ...
        <div class="tips"
             style="margin-top: 20px;">重置登錄賬號後，員工需用新賬號登錄。請及時告知員工，確保正常使用</div>
      </div>
      <span slot="footer"
            class="dialog-footer">
        <el-button @click="()=>{resetUserNameVisible = false}">取 消</el-button>
        <el-button type="primary"
                   @click="passUserNameSubmit(resetUserNameForm)">確 定</el-button>
      </span>
    </el-dialog>
      ...
      <el-form :inline="true"
        ...
        <el-form-item :label="item.value"
          ...
          <el-input v-else
                    ...
                    :disabled="dialogTitle == '編輯員工' && item.field == 'username'"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer"
            class="dialog-footer">
...
export default {
  ...
  data() {
    return {
      ...
      /** 列表 */
      fieldList: [
        { field: 'username', value: '手機號（登錄名）', width: '150' },
        { field: 'sex', value: '性別', type: 'select', width: '50' },
        { field: 'email', value: '郵箱', width: '150' },
        { field: 's_name', value: '部門', type: 'select', width: '100' },
        { field: 'post', value: '崗位', width: '150' },
        {
          field: 'parent_name',
          value: '直屬上級',
          type: 'select',
      ...
      dialogRules: {
        ...
        password: [
          ...
          { min: 6, message: '長度至少為6個字符', trigger: 'blur' }
        ],
        username: [
          { required: true, message: '手機號不能為空', trigger: 'blur' },
          {
        ...
        group_id: [
          { required: true, message: '角色不能為空', trigger: 'change' }
        ]
      },
      // 重置登录账号
  ...
  computed: {
    ...
    /** 添加列表 */
    tableList: function() {
      if (this.dialogTitle === '新建員工') {
        return [
          { field: 'username', value: '手機號（登錄名）' },
          { field: 'password', value: '登錄密碼' },
          { field: 'realname', value: '姓名' },
          { field: 'sex', value: '性別', type: 'select' },
          { field: 'email', value: '郵箱' },
          { field: 'structure_id', value: '部門', type: 'select' },
          { field: 'post', value: '崗位' },
          { field: 'parent_id', value: '直屬上級', type: 'select' },
  ...
  methods: {
    ...
    selectionBarClick(type) {
      ...
      if (type === 'lock' || type === 'unlock') {
        ...
        this.$confirm('這些員工賬號將被' + message + ', 是否繼續?', '提示', {
          confirmButtonText: '確定',
          cancelButtonText: '取消',
          type: 'warning'
        })
