  ...
  <div class="list"
    ...
    <div class="list-content">
      <div class="header">
        ...
        <div class="row">
          <p class="row-title">
            ...
            <span v-else
                  class="read"
                  :style="{'color': data.is_read == 0 ? '#005DF7' : '#ccc'}">{{data.is_read == 0 ? '未讀' : '已讀'}}</span>
          </p>
          ...
          <el-tooltip :disabled="!(data.sendUserList.length > 0 || data.sendStructList.length > 0)"
            ...
            <p class="row-title"
              ...
              <span v-if="data.sendStructList">{{data.sendStructList.length}} 個部門，</span>
              <span v-if="data.sendUserList">{{data.sendUserList.length}}個同事</span>
            </p>
          </el-tooltip>
        </div>
          ...
          <el-dropdown @command="handleCommand"
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-if="data.permission.is_update"
                                command="edit">編輯</el-dropdown-item>
              <el-dropdown-item v-if="data.permission.is_delete"
                                command="delete">刪除</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </div>
      </div>
      <div class="text">
        <p class="row"
           v-if="data.content">
          <span class="title">{{data.category_id == 1 ? "今日工作內容" : data.category_id == 2 ? "本週工作內容" : "本月工作內容"}}：</span>{{data.content}}</p>
        <p class="row"
           v-if="data.tomorrow">
          <span class="title">{{data.category_id == 1 ? "明日工作內容" : data.category_id == 2 ? "下週工作內容" : "下月工作內容"}}：</span>{{data.tomorrow}}</p>
        <p class="row"
           v-if="data.question">
          <span class="title">遇到的問題：</span>{{data.question}}</p>
      </div>
      <div class="accessory">
    ...
    <div class="footer">
      <el-button type="primary"
                 ...
                 @click="commentBtn(data)">回復</el-button>
    </div>
    <!-- 底部评论 -->
    <div class="comment-box"
