...
export default {
  ...
  data() {
    return {
      ...
      formList: [],
      dateList: [
        { label: '今日工作內容', model: 'content' },
        { label: '明日工作內容', model: 'tomorrow' },
        { label: '遇到的問題', model: 'question' }
      ],
      weekList: [
        { label: '本週工作內容', model: 'content' },
        { label: '下週工作內容', model: 'tomorrow' },
        { label: '遇到的問題', model: 'question' }
      ],
      monthList: [
        { label: '本月工作內容', model: 'content' },
        { label: '下月工作內容', model: 'tomorrow' },
        { label: '遇到的問題', model: 'question' }
      ],
      imageFileList: [],
      fileList: [],
  ...
  mounted() {
    ...
    if (this.formData.category_id) {
      switch (this.formData.category_id) {
        case 1:
          this.tabsData = [{ label: '日報', key: '1' }]
          this.formList = this.dateList
          break
        case 2:
          this.tabsData = [{ label: '週報', key: '1' }]
          this.formList = this.weekList
          break
        case 3:
          this.tabsData = [{ label: '月報', key: '1' }]
  ...
  methods: {
    ...
    submitBtn() {
      if (
        ...
        this.$message.error('內容至少填寫一項')
      }
    },
    beforeRemove() {
      return this.$confirm('此操作將永久刪除該圖片, 是否繼續？')
    },
    // 图片和附件
    // 上传图片
