<template>
  <div class="schedule-calendar">
    <div class="title">
      <span>日程</span>
      <div class="rt" @click="addSchedule">
    ...
    <div>
      ...
      <div v-loading="loading">
        ...
        >
          ...
          <div>
            <span class="time"
              >{{ item.start_time | moment("YYYY-MM-DD") }} -
              {{ item.end_time | moment("YYYY-MM-DD") }}</span
            >
            <template v-if="item.ownList.length != 0">
              <span v-for="(k, j) in item.ownList" :key="j">{{
                item.ownList.length - 1 == j ? k.realname : k.realname + "、"
              }}</span>
            </template>
          </div>
        </div>
        <p v-if="scheduleList.length >= 1" @click="seeMore" class="see-more">
          查看更多
        </p>
      </div>
    </div>
