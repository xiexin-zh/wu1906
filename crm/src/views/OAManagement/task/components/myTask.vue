<template>
  <div class="my-task">
    <slot name="searchInput"></slot>
    <div class="select-box">
      <!-- 筛选 -->
      <div class="select-group"
           v-if="listType == 'subtask'">
        <label>任務類型</label>
        <el-select v-model="fromData.type"
                   @change="selectChange"
                   placeholder="請選擇">
          <el-option v-for="item in subordinateOption"
                     :key="item.key"
                     :label="item.label"
                     :value="item.key">
          </el-option>
        </el-select>
      </div>
      <div class="select-group"
           v-else>
        <label>任務類型</label>
        <el-select v-model="fromData.type"
                   @change="selectChange"
                   placeholder="請選擇">
          <el-option v-for="item in typeOptions"
                     :key="item.key"
                     :label="item.label"
                     :value="item.key">
          </el-option>
        </el-select>
      </div>
      <div class="select-group">
        <label>狀態</label>
        <el-select v-model="fromData.status"
                   @change="selectChange"
                   placeholder="請選擇">
          <el-option v-for="item in statusOptions"
                     :key="item.key"
                     :label="item.label"
                     :value="item.key">
          </el-option>
        </el-select>
      </div>
      <div class="select-group">
        <label>優先級</label>
        <el-select v-model="fromData.priority"
                   @change="selectChange"
                   placeholder="請選擇">
          <el-option v-for="item in priorityOptions"
                     :key="item.key"
                     :label="item.label"
                     :value="item.key">
          </el-option>
        </el-select>
      </div>
      <div class="select-group">
        <label>截止時間</label>
        <el-select v-model="fromData.time"
                   @change="selectChange"
                   placeholder="請選擇">
          <el-option v-for="item in timeOptions"
                     :key="item.key"
                     :label="item.label"
                     :value="item.key">
          </el-option>
        </el-select>
      </div>
      <div class="select-group"
           v-if="listType == 'subtask'">
        <label class="min-width">跟進人</label>
        <el-select v-model="fromData.subUser"
                   @change="selectChange"
                   placeholder="請選擇">
          <el-option v-for="item in subUserListData"
                     :key="item.id"
                     :label="item.realname"
                     :value="item.id">
          </el-option>
        </el-select>
      </div>
    </div>
    <div class="list-box-container">
      <div class="list-box">
        <task-cell v-for="(item, index) in list"
                   :key="index"
                   :data="item"
                   :dataIndex="index"
                   @on-handle="taskCellHandle"></task-cell>
      </div>
      <p class="load">
        <el-button type="text"
                   :loading="loadMoreLoading">{{loadMoreLoading ? '加載更多' : '沒有更多了'}}</el-button>
      </p>
    </div>
    <!-- 详情 -->
    <particulars v-if="taskDetailShow"
                 ref="particulars"
                 :id="taskID"
                 :detailIndex="detailIndex"
                 @on-handle="detailHandle"
                 @close="closeBtn">
    </particulars>
  </div>
</template>

<script>
import particulars from '../components/particulars'
import listTaskDetail from '../mixins/listTaskDetail.js'
import TaskCell from './taskCell'

export default {
  components: {
    TaskCell,
    particulars
  },
  data() {
    return {
      fromData: {
        type: '',
        status: '',
        priority: 'all',
        time: '',
        subUser: ''
      },
      // 任务类型
      typeOptions: [
        { label: '全部', key: '' },
        { label: '我負責的', key: 'mymain' },
        { label: '我創建的', key: 'mycreate' },
        { label: '我參與的', key: 'myown' }
      ],
      // 下属任务类型
      subordinateOption: [
        { label: '全部', key: '' },
        { label: '下屬負責的', key: 'mymain' },
        { label: '下屬創建的', key: 'mycreate' },
        { label: '下屬參與的', key: 'myown' }
      ],
      // 状态
      statusOptions: [
        { label: '全部', key: '' },
        { label: '正在進行', key: '1' },
        { label: '已完成', key: '5' }
      ],
      // 优先级
      priorityOptions: [
        { label: '全部', key: 'all' },
        { label: '高', key: '3' },
        { label: '中', key: '2' },
        { label: '低', key: '1' },
        { label: '無', key: '0' }
      ],
      // 截至时间
      timeOptions: [
        { label: '全部', key: '' },
        { label: '今天到期', key: '1' },
        { label: '明天到期', key: '2' },
        { label: '一週到期', key: '3' },
        { label: '一個月到期', key: '4' }
      ]
    }
  },
  mixins: [listTaskDetail],
  props: {
    listType: '',
    list: Array,
    // 负责人
    subUserListData: {
      type: Array,
      default: () => {
        return []
      }
    },
    loadMoreLoading: false
  },
  mounted() {
    document
      .getElementById('workbench-main-container')
      .addEventListener('click', this.taskShowHandle, false)
  },
  methods: {
    selectChange(val, key) {
      this.$emit('selectChange', { type: this.listType, data: this.fromData })
    },
    // 点击空白处关闭详情
    taskShowHandle(e) {
      if (
        this.$refs.particulars &&
        !this.$refs.particulars.$el.contains(e.target)
      ) {
        let hidden = true
        let items = document.getElementsByClassName('list-box')
        for (let index = 0; index < items.length; index++) {
          const element = items[index]
          if (element.contains(e.target)) {
            hidden = false
            break
          }
        }
        this.taskDetailShow = !hidden
      }
    },
    taskCellHandle(data) {
      if (data.type == 'view') {
        this.showDetailView(data.data.item, data.data.index)
      }
    }
  }
}
</script>

<style scoped lang="scss">
.my-task {
  flex: 1;
  display: flex;
  flex-direction: column;
  overflow-y: hidden;
  .wukong {
    vertical-align: middle;
  }
  .select-box {
    margin-top: 22px;
    margin-bottom: 20px;
    // margin-right: -33px;
    max-width: 855px;
    overflow: hidden;
    .select-group {
      margin-bottom: 12px;
      width: 190px;
      float: left;
      label {
        color: #999;
        margin-right: 10px;
      }
      .el-select /deep/ {
        width: 116px;
        .el-input__inner {
          height: 30px;
          line-height: 30px;
        }
      }
      .min-width {
        width: 56px;
        display: inline-block;
      }
    }
    .select-group:nth-child(2) {
      padding-left: 15px;
    }
    .select-submit {
      float: right;
      margin-right: 10px;
    }
  }
  .list-box-container {
    flex: 1;
    overflow-y: auto;
  }
}

.load {
  color: #999;
  font-size: 13px;
  margin: 0 auto 15px;
  text-align: center;
  .el-button,
  .el-button:focus {
    color: #ccc;
    cursor: auto;
  }
}
</style>
