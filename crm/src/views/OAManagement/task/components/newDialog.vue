...
export default {
  ...
  data() {
    return {
      ...
      formList: [
        { label: '任務名稱', field: 'name' },
        { label: '跟進人', field: 'main_user_name', type: 'popover' },
        { label: '開始時間', field: 'start_time', type: 'time' },
        { label: '結束時間', field: 'end_time', type: 'time' },
        { label: '優先級', field: 'priority', type: 'priority' },
        { label: '任務描述', field: 'description', type: 'textarea' }
      ],
      // 上传附件
      uploadFile: [],
      // 负责人弹出框
      ...
      rules: {
        name: [
          ...
          { max: 50, message: '任務名稱長度最多為50個字符', trigger: 'blur' }
        ]
      },
      // 获取选择的数据id数组
      relevanceAll: {},
