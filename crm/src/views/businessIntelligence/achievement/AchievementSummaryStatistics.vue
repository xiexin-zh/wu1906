<template>
  <div v-loading="loading"
    ...
    <div class="content">
      <div class="content-title">簽約合同數：{{data.count_zong}}個；簽約合同金額：{{data.money_zong}}元；回款金額：{{data.back_zong}}元；未收款金額：{{data.w_back_zong}}元</div>
...
export default {
  ...
  data() {
    return {
      ...
      fieldList: [
        { field: 'type', name: '日期' },
        { field: 'count', name: '簽約合同數（個）' },
        { field: 'money', name: '簽約合同金額（元）' },
        { field: 'back', name: '回款金額（元）' }
      ]
    }
  },
  computed: {},
