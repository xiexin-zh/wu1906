...
export default {
  ...
  computed: {
    title() {
      return {
        customer: '員工戶口成交週期（根據合同下單時間和戶口創建時間計算）',
        address: '地區成交週期（根據合同下單時間和戶口創建時間計算）',
        product: '產品成交週期（根據合同下單時間和戶口創建時間計算）'
      }[this.type]
    }
  },
  data() {
    ...
    getFieldList() {
      return {
        customer: [
          ...
          { field: 'cycle', name: '成交週期（天）' },
          { field: 'customer_num', name: '成交戶口數' }
        ],
        product: [
          { field: 'product_name', name: '產品名稱' },
          { field: 'cycle', name: '成交週期（天）' },
          { field: 'customer_num', name: '成交戶口數' }
        ],
        address: [
          { field: 'address', name: '地區' },
          { field: 'cycle', name: '成交週期（天）' },
          { field: 'customer_num', name: '成交戶口數' }
        ]
      }[this.type]
