<template>
  <div class="cr-body-content">
    <flexbox class="content-header">
      <div v-if="!isRelationShow">場景：</div>
      <el-dropdown
        v-if="!isRelationShow"
        trigger="click"
      ...
      >
        <flexbox>
          <div>{{ sceneInfo ? sceneInfo.name : "請選擇" }}</div>
      ...
      <el-input class="search-container" v-model="searchContent">
        <el-button
          slot="append"
      ...
      <el-button class="create-button" @click="isCreate = true" type="primary"
        >新建</el-button
      >
    </flexbox>
    <el-table
    ...
    <div class="table-footer">
      <el-button @click.native="changePage('up')" :disabled="currentPage <= 1"
        >上一頁</el-button
      >
      <el-button
  ...
  methods: {
    ...
    getDefaultField() {
      if (this.crmType === "leads") {
        return [
          { name: "線索名稱", field: "name", form_type: "leads" },
          { name: "下次聯繫時間", field: "next_time", form_type: "datetime" },
          { name: "最後跟進時間", field: "update_time", form_type: "datetime" },
          { name: "創建時間 ", field: "create_time", form_type: "datetime" }
        ];
      } else if (this.crmType === "customer") {
        return [
          { name: "戶口名稱", field: "name", form_type: "customer" },
          { name: "下次聯繫時間", field: "next_time", form_type: "datetime" },
          { name: "最後跟進時間", field: "update_time", form_type: "datetime" },
          { name: "創建時間 ", field: "create_time", form_type: "datetime" }
        ];
        ...
        return [
          ...
          { name: "手機", field: "mobile", form_type: "mobile" },
          { name: "電話", field: "telephone", form_type: "text" },
          { name: "是否關鍵決策人", field: "decision", form_type: "text" },
          { name: "職務", field: "post", form_type: "text" }
        ];
      } else if (this.crmType === "business") {
        return [
          { name: "商機名稱", field: "name", form_type: "text" },
          { name: "商機金額", field: "money", form_type: "text" },
          { name: "戶口名稱", field: "customer_id", form_type: "text" },
          { name: "商機狀態組 ", field: "type_id", form_type: "text" },
          { name: "狀態 ", field: "status_id", form_type: "text" }
        ];
      } else if (this.crmType === "contract") {
        return [
          { name: "合同編號", field: "num", form_type: "text" },
          { name: "合同名稱", field: "name", form_type: "text" },
          { name: "戶口名稱", field: "customer_id", form_type: "text" },
          { name: "合同金額", field: "money", form_type: "text" },
          { name: "開始日期", field: "start_time", form_type: "text" },
          { name: "結束日期", field: "end_time", form_type: "text" }
        ];
      } else if (this.crmType === "product") {
        return [
          { name: "產品名稱", field: "name", form_type: "text" },
          { name: "單位", field: "unit", form_type: "text" },
          { name: "價格", field: "price", form_type: "text" },
          { name: "產品類別", field: "category_id", form_type: "text" },
          { name: "狀態 ", field: "status", form_type: "text" }
        ];
      }
    },
    /** 格式化字段 */
