<template>
  <div>
    <div class="handel-header">
      <el-popover v-model="showPopover"
        ...
        <el-button slot="reference"
                   type="primary"
                   @click="showSelectView=true">添加產品</el-button>
      </el-popover>
    </div>
    ...
    <flexbox class="handle-footer">
      <div class="discount-title">整單折扣（%）：</div>
      <el-input style="width: 80px"
                v-model="discount_rate"
                @blur="discount_rate || (discount_rate = 0)"
                @input="rateChange"
                placeholder="請輸入"></el-input>
      <div class="total-info">已選中產品：
        <span class="info-yellow">{{productList.length}}</span>&nbsp;種&nbsp;&nbsp;總金額：
        <el-input style="width: 80px"
                  v-model="total_price"
                  @input="totalPriceChange"
                  @blur="total_price || (total_price = 0)"
                  placeholder="請輸入"></el-input>&nbsp;元
      </div>
    </flexbox>
  </div>
</template>
