    ...
    <div class="dataMsg">
      <el-table :data="statisData.userResult" class="border-table">
        ...
        <el-table-column label="创建时间" min-width="160">
          <template slot-scope="{ row }">{{ row.s_creattime | formatTime }}</template>
        </el-table-column>
        <el-table-column
...
export default class extends Vue {
  ...
  exportExcelPart(index: number) {
    ...
    window.open(
      `${url}/show/sourcedataExport?token=${token}&n_id=${nId}&current=${index}&page_size=${pageSize}`
    );
  }
