<template>
  <div v-loading.fullscreen.lock="loading" class="cross">
...
export default class extends Vue {
  ...
  async crossAnalysis () {
    if(!this.searchParams.x_id){
      return this.$message.error('请选择自变量');
    }
    if(!this.searchParams.y_id){
      return this.$message.error('请选择因变量');
    }
    this.loading = true
