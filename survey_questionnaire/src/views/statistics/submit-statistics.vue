<template>
  <div v-loading.fullscreen.lock="loading" class="submitStatistics">
    <el-form :model="searchParams" inline>
      ...
      <el-form-item prop="password">
        <el-select v-model="searchParams.u_class" placeholder="请选择组织单位">
...
export default class extends Vue {
  ...
  handleExport () {
    ...
    const u_class = this.searchParams.u_class
    const current = this.pageParams.current
    const page_size = this.pageParams.page_size
    window.location.href = `${api}/show/exportStatis?token=${token}&n_id=${nId}&status=${status}&u_class=${u_class}&current=${current}&current=${current}&page_size=${page_size}`
  }
