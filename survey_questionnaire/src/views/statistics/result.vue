<template>
  <div v-loading.fullscreen.lock="loading" class="result">
    ...
    >
      ...
      <el-table
        v-if="question.type === questionType.SINGLE_CHOICE || question.type === questionType.MULTIPLE_CHOICE || question.type === questionType.DROP_DOWN"
      ...
      <div v-if="question.type === questionType.SINGLE_TEXT || question.type === questionType.TEXT_MUILTIPLE || question.type === questionType.DATE_PICKER">
        <el-alert
        ...
        >
          ...
          <el-table-column prop="userNumber" label="编号" align="center" width="150" />
...
export default class StatisticsComponent extends Vue {
  ...
  downloadXls (index: number) {
    ...
    if (this.questions[index].type !== questionType.SINGLE_TEXT && this.questions[index].type !== questionType.TEXT_MUILTIPLE && this.questions[index].type !== questionType.DATE_PICKER) {
      option.datas = {
      ...
      data = formatJson(option.datas.sheetFilter, this.questions[index].options)
    } else {
      option.datas = {
        sheetData: this.questions[index].answerList,
        sheetName: 'sheet',
        sheetFilter: ['userNumber', 'userName', 'content'],
        sheetHeader: ['编号', '姓名', '提交内容'],
        columnWidths: [20, 20, 40]
      }
      data = formatJson(option.datas.sheetFilter, this.questions[index].answerList)
    }
    exportJson2Excel(
  ...
  getChartsData (questions: any[]) {
    questions.forEach((item: StatisticQuestionItem, quesIndex: number) => {
      if (item.type === questionType.SINGLE_CHOICE || item.type === questionType.MULTIPLE_CHOICE || item.type === questionType.DROP_DOWN) {
        const tempObj: any = {
  ...
  async fetchData () {
    ...
    if (res.success) {
      ...
      this.questions = res.data!.questions.map((item) => {
        return {
          ...item,
          partOfAnswerList: (item.type === questionType.SINGLE_TEXT || item.type === questionType.TEXT_MUILTIPLE) ? item.answerList.slice(0, 100) : item.type === questionType.DATE_PICKER ? item.answerList : []
        }
      })
