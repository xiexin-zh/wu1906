    ...
    <div v-else class="qui-fx qui-fx-ver qui-fx-f1">
      ...
      <table-list :page-list="pageList" :columns="columns" :table-list="roleList">
        <template v-slot:totalNums="totalNum">
          <total-show
            :total="totalNum.record.userSum"
            @click.native="addUser(totalNum.record.id)"
          ></total-show>
        </template>
        <template v-slot:actions="action">
          <a-tooltip placement="topLeft" title="菜单权限">
            <a-button
              size="small"
              class="power-action-btn"
              icon="lock"
              @click.stop="menuPower(action.record)"
            ></a-button>
          </a-tooltip>
          <a-tooltip placement="topLeft" title="编辑">
            <a-button
              ...
              icon="form"
              @click.stop="addRole(1, action.record)"
            ></a-button>
          </a-tooltip>
          <a-popconfirm
            placement="left"
            okText="确定"
            cancelText="取消"
            @confirm="deleteRole(action.record)"
          >
