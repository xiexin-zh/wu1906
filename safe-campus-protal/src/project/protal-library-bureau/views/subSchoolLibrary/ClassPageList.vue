  ...
  <div class="danger-detail page-layout bg-fff qui-fx-ver">
    <div class="content pos-box">
      ...
      <div class="detail-info u-fx-wp">
        <div class="u-mar-b10 u-mar-l10" v-for="(i, index) in detailInfo" :key="index">
          <a-card hoverable style="width: 300px;">
            <img style="height: 180px;" slot="cover" alt="example" :src="i.thumbnailUrl" />
            <a-card-meta :title="i.name">
              <template slot="description">
                <div class="u-font-01">课程类型：{{ i.classType }}&nbsp;&nbsp;&nbsp;适用学段：{{ i.section }}</div>
              </template>
            </a-card-meta>
          </a-card>
        </div>
...
export default {
  ...
  async mounted() {},
  methods: {
    ...mapActions('home', ['treeView', 'claroomUnderList', 'secondCategory', 'firstCategory']),
    moment,
    // 资源类型获取一级分类
    ...
    async showDetail(tag = false) {
      ...
      const res = await this.claroomUnderList(req)
      if (tag) {
