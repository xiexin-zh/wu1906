...
import { templateSet } from '../../assets/js/tableColumns'
import { temSetFormData } from '../../assets/js/submitLable'
...
export default {
  ...
  methods: {
    ...
    async getTemplateType() {
      const res = await this.TempTypeBymsg('2')
    ...
    async delAll() {
      const that = this
      if (this.chooseList.length === 0) return this.$message.error('请先选择模板配置')
      this.$confirm({
        title: '提示',
