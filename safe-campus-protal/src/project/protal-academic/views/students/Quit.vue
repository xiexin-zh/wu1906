...
export default {
  ...
  created() {
    ...
    this.columns[5].title = this.isHighSchool ? '退学日期' : '辍学日期'
    this.columns[6].title = this.isHighSchool ? '退学去向' : '辍学去向'
    this.columns[7].title = this.isHighSchool ? '退学原因' : '辍学原因'
    this.formData[3].label = this.isHighSchool ? '退学日期' : '辍学日期'
    this.formData[4].label = this.isHighSchool ? '退学去向' : '辍学去向'
    this.formData[5].label = this.isHighSchool ? '退学原因' : '辍学原因'
  },
  mounted() {
    this.showList()
  },
  methods: {
    ...mapActions('home', ['getDropoutList', 'addDropout', 'editDropout', 'delChangeList']),
    searchForm(values) {
      const req = {
    ...
    async submitForm(values) {
      ...
      const res = this.type ? await this.editDropout(req) : await this.addDropout(req)
      if (res && res.code === 200) {
    ...
    async showList(obj = {}) {
      ...
      const res = await this.getDropoutList(req)
      if (!res.data.list) {
        this.userList = []
        return
      }
    ...
    confirmSelect(item) {
      ...
      const { userName, userCode, classCode, gradeCode, id, userId, schoolYearId, gradeName, className, workNo } = item
      this.studentInfo = {
        userName: userName || '',
        workNo: workNo || '',
        gradeName: gradeName || '',
