...
const projectName = 'protal-academic' // 此处写项目名作为存储值，避免不同项目冲突
const localData = window.localStorage.getItem(projectName) || '{}'
const getState = (state, val) => {
  return JSON.parse(localData)[state] || val
}
const schoolType = JSON.parse(window.sessionStorage.getItem('loginInfo')).schoolType
const home = {
  namespaced: true,
  state: {
    ...
    isHighSchool: schoolType === '8' || schoolType === '9',
    welComeInfo: {}
  },
  actions: {
    ...actions,
    updateWelComeInfo({ commit, state }, params = '') {
      if (params !== '') {
        commit('setWeleComeInfo', params)
      }
    }
  },
  mutations: {
    ...
    setWeleComeInfo(state, data) {
      state.welComeInfo = data
    }
  }
}
