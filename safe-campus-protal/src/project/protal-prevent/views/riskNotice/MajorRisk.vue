<template>
  <div class="post-card page-layout qui-fx-ver">
    <div class="fill-top">
      <div class="u-mar-l40">重大风险点公告</div>
    </div>
    <div class="qui-fx-f1 qui-fx-ver">
      ...
      <div class="qui-fx-f1 info u-padd-l40 u-padd-r40" v-if="detailInfo && JSON.stringify(detailInfo) !== {}">
        <div class="u-tx-c u-font-2 u-mar-b10">重大风险公告栏</div>
...
export default {
  ...
  mounted() {
    this.reqUrl = `${hostEnv.protal_api_10050}/file/freeUpload?schoolCode=${this.userInfo.schoolCode}`
    this.params.schoolCode = this.userInfo.schoolCode
    this.showDetail()
  },
  methods: {
    ...mapActions('home', ['mostRiskCard', 'riskFileList', 'addRiskFile', 'mostRiskUpload']),
    async showDetail() {
      const req = {
        ...this.pageList,
        schoolCode: this.userInfo.schoolCode
      }
      const res = await this.mostRiskCard(req)
    ...
    uploadPic(info) {
      ...
      if (info.file.status === 'done') {
        ...
        this.mostRiskUpload(req).then(res => {
    ...
    exportWord() {
      if (this.detailInfo && JSON.stringify(this.detailInfo) !== {}) {
        if (this.detailInfo.photoUrl) {
          const url = `${hostEnv.protal_api_10050}/file/downLoad/doc?url=${this.detailInfo.photoUrl}`
          window.open(url)
        } else {
          fetch(`${hostEnv.protal_api_8091}/riskAnnouncement/download/${this.detailInfo.id}`, {
            method: 'Get',
            mode: 'cors'
          })
