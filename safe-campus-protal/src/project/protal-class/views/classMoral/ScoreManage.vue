...
import ScoreDetail from '../../component/ScoreDetail'
...
export default {
  ...
  methods: {
    ...mapActions('home', ['getCategory', 'delCategory', 'getCategoryDetail', 'updateCategory']),
    ...
    async modify(type, text) {
      if (type) {
        const res = await this.getCategoryDetail(text.id)
        const data = res.data
        const eduItemsDtos = res.data.eduItemsDtos
        this.$refs.addScore.scoreId = data.id
        this.$refs.addScore.cycle = data.cycle
        this.$refs.addScore.appForm = data
        this.$refs.addScore.classList = data.classDtoList.map(el => {
          return {
            ...el,
    ...
    categoryGet() {
      ...
      this.getCategory(this.pageList).then(res => {
    ...
    del(type, text) {
      if (type) {
        ...
        this.updateCategory(req).then(res => {
        ...
        // 删除
        this.delCategory(text.id).then(res => {
