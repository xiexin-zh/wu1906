      ...
      <a-form class="u-padd-b20" layout="inline" :form="form">
        ...
        <a-form-item label="资源类型">
          <a-select
            ...
            placeholder="请选择资源类型"
          >
            <a-select-option v-for="(item, index) in firstData" :value="item.id" :key="index">{{
              item.name
            }}</a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item label="资源类别">
          <a-select
            ...
            style="width: 200px;"
            v-decorator="['secondId', ,]"
            placeholder="请选择资源类型"
          >
            <a-select-option v-for="(item, index) in secondData" :value="item.id" :key="index">{{
              item.categoryName
            }}</a-select-option>
          </a-select>
        </a-form-item>
                ...
                <a-row :gutter="[50, 30]" type="flex" justify="center">
                  <a-col :span="15">
                    <a-carousel arrows>
                      ...
                      <div slot="nextArrow" class="custom-slick-arrow" style="right: -50px;zIndex: 10">
            ...
            <a-tab-pane key="2" tab="课后习题" force-render>
              <div class="cont-box u-padd-40 u-padd-b20">
                ...
                >
                  <a-col :span="10">
                    <div class="title">
                      {{ index + 1 }}、{{ item.content }}。【{{ item.topicType }}】({{ item.fraction }}分)
                    </div>
                  </a-col>
                  <a-col :span="24">
                    <div class="u-padd-l20 u-padd-b20">
                      <a-radio-group
                        @change="value => changeAnswer(value, item.answer)"
                        :disabled="false"
                        :options="item.answer"
                      />
                    </div>
            ...
            <a-tab-pane key="3" tab="相关资源" force-render>
              <div class="resource-box  u-padd-40 u-padd-b20">
                <a-row :gutter="[10, 10]" type="flex">
                  <a-col v-for="(item, index) in detailInfo.resourceList" :key="index" :span="24">
                    <div class="comt u-bg-color u-padd-10 u-fx-jsb">
                      ...
                      <div class="rigth u-fx u-padd-r10">
                        <div class="see_icon u-mar-r10 u-fx-ver">
                          查看
                          {{ item.docType }}
                          <a-icon type="eye-invisible" @click="down(0, item.resourceUrl)" />
                          <a-icon v-if="item.docType==='docx'||item.docType==='doc'||item.docType==='xlsx'||item.docType==='xls'||item.docType==='zip'" type="eye-invisible" />
                          <a-icon v-else type="eye" @click="down(0, item.resourceUrl)" />
                        </div>
                        <div class="down_icon u-fx-ver" @click="downloadFile(item.resourceUrl)">
                          下载
...
export default {
  ...
  methods: {
    ...
    // 课后习题答题
    changeAnswer(value, item) {
      const checkValue = value.target.value
      if (item.find(v => v.value === checkValue).selected) {
