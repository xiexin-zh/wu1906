              ...
              <a-collapse-panel :key="`${index + 1}`" v-for="(list, index) in planList">
                ...
                <a-form :form="form[Object.keys(form)[index]]">
                  <a-form-item required label="教案图片" v-bind="formItemLayout">
                    <a-row>
                      <a-col :span="24">
                        <upload-multi :length="10" v-model="list.pictures" :fileInfo="fileInfo"></upload-multi>
                      </a-col>
                    </a-row>
                  </a-form-item>
                  <a-form-item label="教案内容：" v-bind="formItemLayout" :style="{ textAlign: 'center' }" required>
                    <quill-editor
...
export default {
  ...
  methods: {
    ...mapActions('home', ['planSearch', 'planBind']),
    // 添加教案
    addPlan() {
