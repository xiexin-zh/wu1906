  ...
  methods: {
    ...
    async getMenuList(menuType, parentId) {
      const res = await $ajax.post({
        url: `${hostEnv.protal_api_10060}/menu/manage/node/list`,
        params: {
          menuType,
          parentId,
    ...
    async submitForm(values) {
      ...
      try {
        ...
        if (this.isEdit) {
          ...
          await $ajax.post({
            url: `${hostEnv.protal_api_10060}/menu/manage/update`,
            params
          })
          if (this.parentId) {
            this.menuList.find(item => {
          ...
          await $ajax.post({
            url: `${hostEnv.protal_api_10060}/menu/manage/add`,
            params: {
              menuName,
              icon,
    ...
    async delMenu(record, index) {
      await $ajax.get({
        url: `${hostEnv.protal_api_10060}/menu/manage/delete/${record.id}`
      })
      if (record.parentId) {
        this.menuList
          .find(item => {
