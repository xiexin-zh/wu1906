...
/* 富文本编辑图片上传配置 */
const uploadConfig = {
  action: `/protal_api_10050/file/freeUpload?schoolCode=${
    JSON.parse(window.sessionStorage.getItem('loginInfo')).schoolCode
  }`, // 必填参数 图片上传地址
  methods: 'POST', // 必填参数 图片上传方式
  token: '', // 可选参数 如果需要token验证，假设你的token有存放在sessionStorage
  name: 'fileList', // 必填参数 文件的参数名
  size: 1048576, // 可选参数   图片大小，单位为Kb, 1M = 1024Kb
      ...
      fileInput.addEventListener('change', function() {
        ...
        xhr.onload = function(e) {
          if (xhr.status === 200) {
            ...
            self.quill.insertEmbed(length, 'image', res.data[0].url)
