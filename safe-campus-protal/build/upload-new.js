      ...
      setTimeout(() => {
        ...
        inquirer.prompt(promptList).then(answers => {
          ...
          try {
            request.post(
              {
                url: 'http://39.97.164.4:8090/upload-zip?uploadPath=/usr/local/nginx/html/pc-protal/',
                formData: formData
              },
              function(error, response, body) {
                if (!error && response.statusCode === 200) {
                  request.post(
                    {
                      url: 'http://112.125.89.37:8090/upload-zip?uploadPath=/usr/local/openresty/nginx/html/pc-protal/',
                      formData: formData
                    },
                    function(error, response, body) {
                      if (!error && response.statusCode === 200) {
