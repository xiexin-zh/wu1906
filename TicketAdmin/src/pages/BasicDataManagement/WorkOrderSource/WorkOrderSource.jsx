...
class WorkOrderSource extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      lists: [],
  ...
  tableClickEvent = (event, record = {}) => {
    ...
    if (currentClickState === 'delete') {
      Modal.confirm({
        content: `是否确认删除 ${record.name} 工单来源?`,
        onOk: () => {
          delBasicData({
            id: record.id,
            category: 'source',
  ...
  render() {
    ...
    return (
      <div className={publicStyle.pageContent}>
        <PageHeaderWrapper title="工单来源" breadcrumb extra={this.renderExtra()}>
          <main>
            <ShareTable
              loading={loading}
...
WorkOrderSource.propTypes = {};

function mapStateToProps() {
  return {};
}
export default connect(mapStateToProps)(WorkOrderSource);
