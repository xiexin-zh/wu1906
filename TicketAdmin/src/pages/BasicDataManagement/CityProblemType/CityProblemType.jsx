...
import { getChildrenById, delBasicData, getAllCityType } from '@/services/basicDataManagement';
import BasicDataBackup from '../components/BasicDataBackup';
import AddCoreCityTree from '../components/AddCoreCityTree';

class CityProblemType extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedKeys: {},
  ...
  async componentDidMount() {
    await this.getAllCityTypeLists();
  }
  ...
  tableClickEvent = (event, record = {}) => {
    ...
    if (currentClickState === 'delete') {
      Modal.confirm({
        content: `是否确认删除 ${record.name} 市级问题子类型?`,
        onOk: () => {
          delBasicData({
            id: record.id,
            category,
        ...
        <PageHeaderWrapper breadcrumb>
          ...
          {visible && (
            <BasicDataBackup
              ...
              operation={operation}
              refreshTable={this.getAllCityTypeLists}
            />
          )}
          {addTreeVisible && (
            <AddCoreCityTree
              ...
              operation={operation}
              cloneModal={this.cloneModal}
              refresh={this.getAllCityTypeLists}
            />
          )}
        </PageHeaderWrapper>
      </div>
...
CityProblemType.propTypes = {};

function mapStateToProps() {
  return {};
}
export default connect(mapStateToProps)(CityProblemType);
